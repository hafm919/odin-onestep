...
export default class UI {
  ...
  static createTaskOptions(task) {
    ...
    for (let i in icons) {
      if (
        (selectedProject == "your-day" &&
          (icons[i] == "light_mode" || icons[i] == "star")) ||
  ...
  static renderProjects() {
    ...
    for (let i in projectNames) {
      ...
      if (
        project != "your-day" &&
        project != "important" &&
        !ProjectManager.checkIfDeleted(project)
      ) {
  ...
  static createProject() {
    ...
    const currentProjectNames = ProjectManager.getAllProjectNames();
    if (currentProjectNames.indexOf(projectNameInput.value) > -1) {
      projectNameInput.setCustomValidity("Project names needs to be unique");
      projectNameInput.reportValidity();
